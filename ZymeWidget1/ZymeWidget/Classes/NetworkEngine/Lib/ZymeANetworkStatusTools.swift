...
public class ZymeANetworkStatusTools {
    
    private static var _shared: ZymeANetworkStatusTools?
    
    static var shared: ZymeANetworkStatusTools? {
        
        if _shared == nil {
            _shared = ZymeANetworkStatusTools()
        }
        return _shared
    }
    ...
    private init?(host: String) {
        ...
        listener = { (status) in
            ...
            case .reachable(let type):
                zymeNetDebugLog(type)
            case .unknown:
                zymeNetDebugLog("未知网络")
            }
        }
    }
