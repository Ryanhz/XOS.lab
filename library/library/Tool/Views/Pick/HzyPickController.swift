...
protocol HzyPickerViewable {
    var selectedData: Any? { get }
}

class HzyPickController: UIViewController {
    ...
    fileprivate var pickView:  HzyPickerViewable!
    fileprivate var contentView: UIView!
    
    fileprivate var selectedBlock:  ((Any?)->Void)!
    
//    fileprivate lazy var popoverAnimator: HzyPopoverAnimator = HzyPopoverAnimator {[weak self] (presented) in
//    }
    ...
    init<T: HzyPickerViewable>(_ pickView: T, selectedBlock: @escaping (Any?)->Void) where T: UIView {
        
        super.init(nibName: nil, bundle: nil)
    
//        defautHeight += UIScreen.hzy.extraBottom
...
//        popoverAnimator.presentedFrame = CGRect(x: 0, y: screenBounds.height - defautHeight, width: UIScreen.width, height: defautHeight)
    ...
    fileprivate func setupSubviews() {
        ...
        chooseBtn = UIButton()
        chooseBtn.setTitleColor(BaseColor.green_00D2DB, for: .normal)
        ...
        (pickView as! UIView).snp.makeConstraints { (make) in
            ...
            make.bottom.equalTo(contentView).offset(-UIScreen.hzy.extraBottom)
        }
    }
}
