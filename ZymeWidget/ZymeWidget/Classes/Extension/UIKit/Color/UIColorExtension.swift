...
//extension UIColor: ZymeNamespaceWrappable {}

extension ZymeNamespaceWrapper where T: UIColor {
    ...
    public static func color(hex: String) ->UIColor {
        ...
        case let text where text.hasPrefix("0x"):
            hexColorText = text.zyme.substring(from: 2)
        case let text where text.hasPrefix("#"):
            hexColorText = text.zyme.substring(from: 1)
        default:
            break
        }
    ...
    private static func hexColor(text: String, defaultColor: UIColor = UIColor.clear) -> UIColor {
        ...
        var g: UInt32 = 0
        var b: UInt32 = 0
        guard Scanner(string: text.zyme.substring(with: 0..<2)).scanHexInt32(&r),
            Scanner(string: text.zyme.substring(with: 2..<4)).scanHexInt32(&g),
            Scanner(string: text.zyme.substring(with: 4..<6)).scanHexInt32(&b)
            else {
                return defaultColor
        }
    ...
    private static func alphaHexColor(text: String, defaultColor: UIColor = UIColor.clear) -> UIColor {
        let alphaString = text.zyme.substring(with: 0..<2)
        let redString = text.zyme.substring(with: 2..<4)
        let greenString = text.zyme.substring(with: 4..<6)
        let blueString = text.zyme.substring(with: 6..<8)
...
extension UIColor {
    ...
    convenience init(hex: String) {
        self.init(cgColor: UIColor.zyme.color(hex: hex).cgColor)
    }
